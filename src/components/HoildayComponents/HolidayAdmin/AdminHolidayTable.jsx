...
export default function AdminHolidayTable({ requests, onApprove, onReject, filters, selectedStat, onClose }) {
  ...
  if (selectedStat) {
    if (selectedStat === 'approved') {
      filteredRequests = filteredRequests.filter((req) => req.status === 'Ù…Ø¹ØªÙ…Ø¯')
    } else if (selectedStat === 'pending') {
      filteredRequests = filteredRequests.filter((req) => req.status === 'Ù‚ÙŠØ¯ Ø§Ù„Ø§Ù†ØªØ¸Ø§Ø±')
    } else if (selectedStat === 'rejected') {
      filteredRequests = filteredRequests.filter((req) => req.status === 'Ù…Ø±ÙÙˆØ¶')
    }
  }

  if (filters.searchName) {
    filteredRequests = filteredRequests.filter((req) => req.fullName.includes(filters.searchName))
  }

  if (filters.searchMilitaryId) {
    filteredRequests = filteredRequests.filter((req) => req.militaryId.includes(filters.searchMilitaryId))
  }
                  ...
                  return (
                    <tr key={request.id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        ...
                        >
                          {request.penalty ? `⚠ ${request.penalty}` : '✓ بدون عقوبة'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        {request.requestType === 'appeal' ? (
                          <div className="space-y-1">
                            <span className="inline-flex px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-sm font-bold">
                              التماس
                            </span>
                            <p className="text-xs text-gray-500 max-w-[220px]">{request.reason}</p>
                          </div>
                        ) : (
                          <span className="text-gray-500 text-sm">طلب إجازة</span>
                        )}
                      </td>
